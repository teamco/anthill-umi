...
function _getWidget(page, payload) {
  return ((page || {}).widgets || []).find(widget => widget.contentKey === (payload.widget || {}).contentKey);
}
...
export default dvaModelExtend(commonModel, {
  ...
  effects: {
    ...
    * updateColumnCellWidth({payload}, {put, select}) {
      ...
      const _formValues = fromForm(entityForm);
      const cellWidth = page.layout.cellWidth(_formValues.pageWidth, payload.columns);
    ...
    * updatePageWidthCellWidth({payload}, {put, select}) {
      ...
      const _formValues = fromForm(entityForm);
      const cellWidth = page.layout.cellWidth(payload.width, _formValues.gridColumns);
